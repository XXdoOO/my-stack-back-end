...
create table `userVo`
(
    `id`            bigint auto_increment primary key     not null comment '用户名',
...
(
    ...
    `logic_delete` tinyint(1) default 0                 not null comment '逻辑删除，1为删除',
    foreign key (`user_id`) references `userVo` (`id`)
) comment '用户封号记录';
...
(
    ...
    `author_id`    bigint                               not null comment '作者用户名',
    ...
    `logic_delete` tinyint(1) default 0                 not null comment '逻辑删除，1为删除',
    foreign key (`author_id`) references `userVo` (`id`)
...
(
    ...
    `sender_id`    bigint                               not null comment '评论者的id',
    `receive_id`   bigint comment '接收者的id，为null则回复的是根评论',
    ...
    `logic_delete` tinyint(1) default 0                 not null comment '逻辑删除，1为删除',
    foreign key (`blog_id`) references `blog` (`id`),
    foreign key (`sender_id`) references `userVo` (`id`),
    foreign key (`receive_id`) references `userVo` (`id`)
...
insert into userVo(email, password, nickname, register_time, identity, logic_delete)
